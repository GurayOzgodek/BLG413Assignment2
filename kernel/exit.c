...
/* Implementation queue data structure for BFS algorithm  */
...
int enqueue(Queue *queue, struct list_head *item){
...
/* Check whether the process have child or not */
int checkListHead(struct list_head *head)
{
    struct list_head *next = head->next;
    return (next == head) && (next == head->prev);
}

/* If the process are killed, the children of the process is updated nice_value  */
void updateChildNiceValue(struct list_head *childHead)
{
	Queue taskQueue;
	...
	/* add new element childList */
	enqueue(&taskQueue, childHead);
	
	/* do until queue is empty */
    	while (!isEmpty(&taskQueue)) 
    	{
		/* get list_head from queue */
		 headChild = dequeue(&taskQueue);
		 ...
		 list_for_each_entry_safe(p, n, headChild, sibling) {
            	 ...
            	 /* Update nice value each process */
             		...
             		if ( !(checkListHead(&(t->children))))
            		 {
				/* add new element childList */
                		 enqueue(&taskQueue, &(t->children));
             		}
...
void do_exit(long code)
{
    ...
    if ( !(checkListHead(&childHead)))
    {
        updateChildNiceValue(&childHead);
    }
